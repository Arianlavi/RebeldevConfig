...
class ConfigurationParser:
    ...
    def parse_configuration(self, protocol: str, raw_config: str) -> Optional[VPNConfig]:
        ...
        try:
            if protocol == 'vmess':
                parsed = self._parse_vmess(raw_config)
            elif protocol == 'vless':
            ...
            if parsed:
                ...
                if host and len(host.strip()) > 3 and not host.strip().startswith('.'):
                    return VPNConfig(protocol=protocol, host=host.strip(), port=port, 
                                   name=name.strip()[:60], raw_config=raw_config)
        except Exception as e:
            self.logger.log_error(f"Parse exception {protocol}", str(e)[:100])
        
        if not host:
            self.logger.log_error(f"Missing hostname in {protocol.upper()}", f"Config: {raw_config[:60]}...")
        return None

    def _clean_base64(self, b64_str: str) -> str:
        """Clean and prepare base64 string"""
        cleaned = re.sub(r'[^A-Za-z0-9+/=]', '', b64_str.strip())
        padding = (4 - len(cleaned) % 4) % 4
        return cleaned + '=' * padding

    def _safe_decode(self, data: str) -> str:
        try:
            cleaned = self._clean_base64(data)
            decoded_bytes = base64.b64decode(cleaned, validate=False)
            return decoded_bytes.decode('utf-8', errors='ignore').strip()
        except:
            return data.strip()

    def _parse_vmess(self, raw: str) -> Tuple[Optional[str], Optional[int], str]:
        try:
            ...
            if json_match:
                try:
                    ...
                    host = config.get('add') or config.get('server') or ''
                    port = int(config.get('port', 443))
                    name = config.get('ps') or config.get('remark', 'VMESS')
    ...
    def _parse_vless(self, raw: str) -> Tuple[Optional[str], Optional[int], str]:
        ...
        try:
            ...
            name = unquote(name_match.group(1))[:60] if name_match else 'VLESS'
            return host, port, name
        except:
            return None, None, 'VLESS'
    ...
    def _parse_ss(self, raw: str) -> Tuple[Optional[str], Optional[int], str]:
        ...
        try:
            # Split config and remark
            if '#' in raw:
                main_part, remark = raw.split('#', 1)
                name = unquote(remark.strip())[:60]
            else:
                main_part = raw
                name = 'SS'

            b64_part = main_part[5:]
            decoded = self._safe_decode(b64_part)
            ...
            # Clean host:port part
            host_port = after_at.split('?')[0].split('/')[0].strip()
            ...
            self.logger.log_error("SS parse error", str(e)[:80])
            return None, None, 'SS'
    ...
    def _parse_trojan(self, raw: str) -> Tuple[Optional[str], Optional[int], str]:
        ...
        try:
            ...
            host_port_part = uri[last_at + 1:].split('?')[0].split('/')[0].strip()
            ...
            name_match = re.search(r'#(.+)$', raw)
            name = unquote(name_match.group(1))[:60] if name_match else 'TROJAN'
            
            return host, port, name
        except Exception as e:
            self.logger.log_error("Trojan parse error", str(e)[:80])
            return None, None, 'TROJAN'
...
class ImprovedPerformanceTester:
    ...
    async def test_ping_performance(self, host: str) -> Tuple[Optional[int], Optional[int], Optional[float]]:
        if not host or len(host) < 4 or host in ['localhost', '127.0.0.1', '0.0.0.0']:
    ...
    async def comprehensive_performance_test(self, config: VPNConfig) -> VPNConfig:
        if not config.host or len(config.host) < 4:
...
class ImprovedVPNScanner:
    ...
    def _load_cache(self) -> set:
        ...
        if cache_path.exists():
            try:
                ...
                valid_cache = {ts: h for ts, h in cache.items() 
                             if datetime.fromisoformat(ts) > now - timedelta(days=self.config.CONFIG_RETENTION_DAYS)}
                with open(cache_path, 'w', encoding='utf-8') as f:
                    json.dump(valid_cache, f)
                return set(valid_cache.values())
            except:
                pass
        return set()
    ...
    def _save_cache(self):
        cache_path = Path(self.config.OUTPUT_DIRECTORY) / self.config.CACHE_FILE
        try:
            cache = {c.last_tested.isoformat(): c.config_hash 
                    for lst in self.validated_configs.values() 
                    for c in lst if c.is_valid}
            with open(cache_path, 'w', encoding='utf-8') as f:
                json.dump(cache, f)
        except:
            pass
    ...
    async def _fetch_all_configs(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                async with session.get(url, headers={'User-Agent': 'RebelDev-Scanner/1.1.0'}) as resp:
                    if resp.status == 200:
                        ...
                        self.logger.log_performance("File fetched", f"{len(lines)} lines")
            ...
            self.logger.log_error("Fetch failed", str(e))
    ...
    def save_enhanced_configurations(self):
        ...
        for proto, configs in self.validated_configs.items():
            ...
            self.logger.log_performance("Saved", f"{proto}: {len(configs)} configs")
